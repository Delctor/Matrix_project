		...
		if constexpr (returnTransposed)
		{
			...
			else
			{
				for (size_t j = 0; j < cols; j++)
				{
					...
					for (size_t i = 0; i < finalPosRows; i += 4)
					{
						__m256d a = _mm256_setr_pd(data1[i * actualCols + j],
							data1[(i + 1) * actualCols + j],
							data1[(i + 2) * actualCols + j],
							data1[(i + 3) * actualCols + j]);
		...
		else
		{
			...
			if constexpr (thisTransposed)
			{
				for (size_t j = 0; j < finalPosCols; j += 4)
				{
					...
					for (size_t i = 0; i < rows; i++)
					{
						__m256d a = _mm256_setr_pd(data1[j * actualRows + i],
							data1[(j + 1) * actualRows + i],
							data1[(j + 2) * actualRows + i],
							data1[(j + 3) * actualRows + i]);
						_mm256_store_pd(&dataResult[i * cols + j], _mm256_add_pd(a, b));
					}
				}
				for (size_t j = finalPosCols; j < cols; j++)
		...
		else
		{
			...
			for (size_t j = finalPosCols; j < cols; j++)
			{
				...
				for (size_t i = 0; i < finalPosRows; i += 4)
				{
					...
					_mm_store_sd(&data1[i * cols + j], val2);
					val2 = _mm_shuffle_pd(val2, val2, 1);
					_mm_store_sd(&data1[(i + 1) * cols + j], val2);

					_mm_store_sd(&data1[(i + 2) * cols + j], val1);
					val1 = _mm_shuffle_pd(val1, val1, 1);
					_mm_store_sd(&data1[(i + 3) * cols + j], val1);
				}
				for (size_t i = finalPosRows; i < rows; i++)
				{
